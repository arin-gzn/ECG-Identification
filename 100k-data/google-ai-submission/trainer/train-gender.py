...
def train_and_evaluate_100k(args):
    ...
    all_data_loaded_df=all_data_loaded_df[all_data_loaded_df.iloc[:,3604]!=2]
    ...
    train_df_race_combination=train_df.groupby(train_df.columns[3604]).size().reset_index(name='count')
    print(train_df_race_combination.head())

    test_df_race_combination=test_df.groupby(test_df.columns[3604]).size().reset_index(name='count')
    ...
    trainY=train_nparr[:,3604]

    testX=test_nparr[:,:3600].reshape((-1, 12, 300,1))
    testY=test_nparr[:,3604]
