...
def train_and_evaluate(args):
    ...
    all_data_loaded_df.loc[all_data_loaded_df.iloc[:,3657]>=70.0,3660 ]=1
    all_data_loaded_df.loc[all_data_loaded_df.iloc[:,3657]<=40.0,3660 ]=0
    ...
    train_df_race_combination=train_df.groupby(train_df.columns[3660]).size().reset_index(name='count')
    print(train_df_race_combination.head())

    test_df_race_combination=test_df.groupby(test_df.columns[3660]).size().reset_index(name='count')
    ...
    trainY=train_nparr[:,3660]
    testY=test_nparr[:,3660]

    train_nparr=train_nparr[:,:3660]
    test_nparr=test_nparr[:,:3660]
